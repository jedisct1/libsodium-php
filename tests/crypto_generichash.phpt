...
$normalHash = Sodium::crypto_generichash($randomBytes, '0123456789abcdef');

$partialState = Sodium::crypto_generichash_init('0123456789abcdef');
for ($i = 0; $i < 8; $i++) {
	Sodium::crypto_generichash_update($partialState, substr($randomBytes, $i * 16, 16));
}
$hashByParts = Sodium::crypto_generichash_final($partialState);
var_dump($normalHash === $hashByParts);
