...
#ifdef crypto_pwhash_SALTBYTES
    REGISTER_LONG_CONSTANT("Sodium\\CRYPTO_PWHASH_ALG_ARGON2I13",
                           crypto_pwhash_ALG_ARGON2I13, CONST_CS | CONST_PERSISTENT);
# ifdef crypto_pwhash_ALG_ARGON2ID13
    REGISTER_LONG_CONSTANT("Sodium\\CRYPTO_PWHASH_ALG_ARGON2ID13",
                           crypto_pwhash_ALG_ARGON2ID13, CONST_CS | CONST_PERSISTENT);
# endif
    REGISTER_LONG_CONSTANT("Sodium\\CRYPTO_PWHASH_ALG_DEFAULT",
...
PHP_FUNCTION(crypto_pwhash)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lssll|l",
                              &hash_len,
                              &passwd, &passwd_len,
                              &salt, &salt_len,
    ...
    if (alg != crypto_pwhash_ALG_ARGON2I13
        ...
        zend_error(E_RECOVERABLE_ERROR, "unsupported password hashing algorithm");
    }
    if (passwd_len <= 0) {
        zend_error(E_WARNING, "empty password");
    }
